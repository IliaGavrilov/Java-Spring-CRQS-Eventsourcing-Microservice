...
public class BankAccountController {
    ...
    public String createBankAccount(@Valid @RequestBody CreateBankAccountRequestDTO dto) {
        final var id = commandService.handle(new CreateBankAccountCommand(UUID.randomUUID().toString(), dto.email()));
        log.info("Created bank account id: {}", id);
        return id;
    }

    @PostMapping(path = "/deposit/{aggregateId}")
    public void depositAmount(@Valid @RequestBody DepositAmountRequestDTO dto, @PathVariable String aggregateId) {
        commandService.handle(new DepositAmountCommand(aggregateId, dto.amount()));
    }

    @PostMapping(path = "/credit/{aggregateId}")
    public void creditAmount(@Valid @RequestBody CreditAmountRequestDTO dto, @PathVariable String aggregateId) {
        commandService.handle(new CreditAmountCommand(aggregateId, dto.amount()));
    }

    @PostMapping(path = "/debit/{aggregateId}")
    public void debitAmount(@Valid @RequestBody DebitAmountRequestDTO dto, @PathVariable String aggregateId) {
        commandService.handle(new DebitAmountCommand(aggregateId, dto.amount()));
    }

    @PostMapping(path = "/email/{aggregateId}")
    public void changeEmail(@Valid @RequestBody ChangeEmailRequestDTO dto, @PathVariable String aggregateId) {
